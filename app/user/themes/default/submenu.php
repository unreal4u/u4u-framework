...
if ($app->loggedIn) {
  $r['submenu'][] = array('link' => 'passwd/', 'txt' => __('Change Password'),'spec' => TRUE);
  $r['submenu'][] = array('link' => 'logout/', 'txt' => __('Logout'), 'spec' => TRUE);
}
if (!empty($r['submenu'][0])) {
  ...
  foreach($r['submenu'] AS $a) {
    ...
    if ($tmpPag !== FALSE) $pagina = $tmpPag;
    if ($a['link'] == $pagina) $special = 'sub_current';
    elseif (!empty($a['spec'])) $special = 'sub_special';
    else $special = '';
